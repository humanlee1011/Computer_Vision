...
void HoughLine(CImg<float>& img, CImg<float>& edge, CImg<float>& HoughSpace, CImg<float>& result,
  float in_thresh, float out_thresh, vector<pair<int, int> >& Points) {
  ...
  for (int i = 0; i < nonmax.size() && i < 4; i++) {
    ...
    // for (int j = 0; j < i; j++) {
    //   pair<int, int> point;
    //   float theta1 = nonmax[j].first * theta_step;
    //   float R1 = diagonal * (float(nonmax[j].second) / OFFSET_N - 0.5f);
    
    //   if (getCrossPoint(theta, R, theta1, R1, point)) {
    //     cout << point.first << " " << point.second << endl;
    //     point.first += width2;
    //     point.second += height2;
  ...
  for (int i = 0; i < Points.size(); i++) {
    ...
    output2.draw_circle(Points[i].first, Points[i].second, 5, red);
  }
  // detect point
  cimg_forXY(count, i, j) {
      if (count(i, j) > 1 || (i < width - 1 && j < height - 1 && 
        count(i, j) == 1 && count(i, j + 1) == 1 && count(i + 1, j) == 1 && count(i + 1, j + 1) == 1)) {
        float red[3] = {255.0f, 0.0f, 0.0f};
...
// 将极坐标转换成直角坐标求交点
bool getCrossPoint(float theta1, float r1, float theta2, float r2, pair<int, int>& point) {
  float k1 = 0, 
        k2 = 0, 
        b1 = 0, 
        b2 = 0;
  k1 = -sin(theta1) / cos(theta1);
  k2 = -sin(theta2) / cos(theta2);
  ...
  if (k1 == k2)
    return false;
  point.first = (b2 - b1) / (k1 - k2);
  point.second = k2 * point.first + b2;
  return true;
}

float getK(float theta) {
  float k = 0;
  if (theta < PI / 2)
    k = tan(theta + PI / 2);
  else if (theta > PI / 2)
                ...
                float thresh, int halfwidth)
{
  ...
  cimg_forXY(img, x, y) {
      ...
      if (value < thresh)
       continue;
     // 如果在同一个区域出现两个相等的local maximum, 增加判断语句
      bool ismax = true;
      for (int ny = y - halfwidth; ny <= y + halfwidth; ny++) {
        for (int nx = x - halfwidth; nx <= x + halfwidth; nx++) {
          if (ny < 0 || ny >= img._height || nx < 0 || nx >= img._width)
            continue;
          //如果 nx在x的后面（未遍历）, 需要小于等于该值
          if (nx >= x)
            ismax = ismax && (img(nx, ny) <= value);
          else  // 如果nx在x的前面，则需要完全小于该值
            ismax = ismax && (img(nx, ny) < value);
        }}
      if (!ismax) continue;
