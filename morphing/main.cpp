...
int main() {
  ...
  for (int i = 1; i <= 6; i++) {
    ...
    unsigned char* edge = canny(unsignedimg, img._width, img._height, 4.0f, 7.0f, 6.5f);
    CImg<float> edgeCimg(edge, img._width, img._height);
    //edgeCimg.display("edge");
    CImg<float> hough, resultHough, resultMorph;
    vector<pair<int, int> > Point;
    HoughLine(img, edgeCimg, hough, resultHough, 200.0f, 0.5f, Point);
    perspectiveTransform(img, resultMorph, Point);
